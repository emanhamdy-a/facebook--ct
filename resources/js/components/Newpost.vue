<template>
  <div class="bg-white shadow w-2/3 rounded p-4">
    <div class="flex justify-between items-center">
      ...
      <div class="flex-1 flex mx-4">
        <input v-model="postMessage" type="text" name="body"
          class=" rounded-full focus:outline-none focus:shadow-outline w-full h-8 pl-5 bg-gray-200"
          placeholder="Add a post"
        />
        <transition name="fade">
          <button v-if="postMessage"
            @click="$store.dispatch('postMessage')"
            class=" bg-gray-200 rounded-full ml-2 py-1 px-3 focus:outline-none focus:bg-gray-400"> Post
          </button>
        </transition>
      </div>
      <div>
        <button class="flex justify-center items-center w-10 h-10 bg-gray-200 focus:outline-none focus:bg-gray-400 rounded-full">
